...
async function scrapSMCP() {
    ...
    $('li.box-list__item').each((i, element) => {
        const $element = $(element);
        const offers = {};
        offers.name = $element.find($('a.box-list__item-title')).text().replace(/\s\s+/g, ' ').trim();
        offers.link = $element.find($('a.box-list__item-title')).attr('href');
        offers.company = $element.find($('img.box-list__visual')).attr('alt');
        offers.function = "";
        offers.details = $element.find($('div.box-list__item-infos')).text().replace(/\s\s+/g, ' ').trim();

        if (offers.name !== '') {
            list_offers.push(offers);
        }
    });
    const spinner = ora('Scrapping SMCP\n');
    spinner.start();
    for (var i = 0; i < list_offers.length; i++) {
        try{
            const { data } = await axios.get(list_offers[i].link);
            const $ = cheerio.load(data);
            list_offers[i].desc = $('section.section').text().replace(/\s\s+/g, ' ').trim();
            idx.add_to_db(list_offers[i]);
        } catch(e) {
            console.log(e.message);
        }
